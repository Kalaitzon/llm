# -*- coding: utf-8 -*-
"""
01_generate_decoys.py  --  Generation of the decoy artefacts
============================================================
[TASK 2] Creation of the fake bait items (decoy artefacts).

WHAT THIS FILE DOES
-------------------
Creates 12 fake but realistic files (decoys) in 7 categories, and places them
inside a simulated filesystem (the decoys/fs/ folder). This virtual filesystem
is exactly what the honeypot later "serves": when the attacker runs e.g.
`cat /opt/app/.env`, the honeypot reads the corresponding file from here. It
also produces artifact_inventory.json, i.e. the catalogue of all the decoys
with their metadata (category, source, role), as Task 2 explicitly requires.

HOW IT WORKS (the steps)
------------------------
1. gen_manual() : writes the 5 manual (template-based) decoys.
2. gen_llm()    : writes the 7 decoys produced with the Claude model.
3. write_prompts() : saves into llm_prompts/ the exact prompt of each LLM decoy
   (documentation of the generation method).
4. write_inventory() : writes the overall catalogue artifact_inventory.json.
The helper write_fs() writes each file to the right path and at the same time
records it in the catalogue.

WHICH ARE LLM AND WHICH ARE MANUAL
----------------------------------
- Claude (7): svc_deploy's .bash_history, app.corp.conf (nginx),
  database.yml, auth.log, .ssh/config, deploy_runbook.md, NOTES_infra.md.
- Manual (5): mgeorgiou's .bash_history, .env, postgresql log,
  .ssh/known_hosts, cron.d/deploy.

HOW THE LLM ARTEFACTS WERE GENERATED
------------------------------------
The content of the LLM artefacts was produced with the Claude model, using
explicit prompts that describe the system profile and ask for realistic,
internally consistent content WITHOUT real secrets. The exact prompt of each
artefact is stored in the llm_prompts/ folder for full documentation and
reproducibility. Thus, the content of the LLM decoys is fixed and embedded in
the script, with no API connection required at run time.

INPUT / OUTPUT
--------------
- Input : the shared profile (00_config.py).
- Output: the decoys/fs/ folder (the 12 files), decoys/artifact_inventory.json
  and the prompt files in llm_prompts/.

SAFETY: no credential is real. Everything points to fictional hosts
(RFC 1918/5737). There are no personal data.
"""
...
# Import the shared profile (the name 00_config is not a valid identifier,
# so we load it dynamically).
cfg = importlib.import_module("00_config")

BASE      = Path(__file__).resolve().parent
DECOY_DIR = BASE / "decoys"
FS_ROOT   = DECOY_DIR / "fs"          # the simulated "/" of the honeypot
...
# ==========================================================================
#  Prompts of the LLM-generated artefacts (Task 2 documentation)
#  Written to the llm_prompts/ folder at run time.
# ==========================================================================
LLM_PROMPTS = {
...
# ==========================================================================
#  Helper: write a file into the simulated filesystem
# ==========================================================================
INVENTORY = []
...
def write_fs(rel_path, content, *, category, source, owner, story,
    ...
    """Write an artefact to decoys/fs/<rel_path> and record it in the
    inventory. rel_path is the ABSOLUTE path as the attacker will see it
    (e.g. /home/mgeorgiou/.bash_history)."""
...
# ==========================================================================
def gen_manual():
    # ---- [manual 1] sysadmin mgeorgiou .bash_history (shell_history) ------
    admin = U["mgeorgiou"]
    bash_admin = "\n".join([
        "cd /opt/app",
    ...
    # ---- [manual 2] app/.env (credentials, KEY=value format) ----------------
    env = (
...
#  The content was produced with the Claude model (prompts in llm_prompts/).
# ==========================================================================
def gen_llm():
...
# timedelta shortcut for use inside the module
